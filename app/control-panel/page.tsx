    ...
    return (
        <Box sx={{ p: 4, maxWidth: 700, margin: 'auto' }}>
            ...
            <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
                ...
                <TextField label="フィールド名" value={scoreLabel} onChange={(e) => setScoreLabel(e.target.value)} fullWidth margin="normal"/>
                <TextField label="値" type="number" value={scoreValue} onChange={(e) => setScoreValue(Number(e.target.value))} fullWidth margin="normal"/>
            </Paper>

            <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
                {/* ... (メッセージ設定UI) ... */}
                <Typography variant="h6" gutterBottom>視聴者向け概要メッセージ</Typography>
                <Box sx={{ my: 2, maxHeight: 150, overflowY: 'auto', border: '1px solid #ddd', borderRadius: '4px', p: 1 }}>
                    {messages.length === 0 ? (<Typography variant="body2" color="textSecondary" sx={{ p: 1 }}>メッセージがありません</Typography>) : (
                        messages.map((msg) => (
                            <Box key={msg.id} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 0.5, borderBottom: '1px dotted #eee' }}>
                                <Typography variant="body1" sx={{ flexGrow: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{msg.text}</Typography>
                ...
                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 2 }}>
                    <TextField label="新しいメッセージ" value={currentMessage} onChange={(e) => setCurrentMessage(e.target.value)} fullWidth/>
                ...
                <TextField label="表示秒数 (秒)" type="number" value={transitionDuration} onChange={(e) => setTransitionDuration(Number(e.target.value))} fullWidth margin="normal" inputProps={{ min: 1 }}/>
            </Paper>
            ...
            {status === 'success' && (<Typography color="primary" align="center" sx={{ mt: 2 }}>✅ データを更新しました！OBS画面に反映されます。</Typography>)}
            {status === 'error' && (<Typography color="error" align="center" sx={{ mt: 2 }}>❌ 更新に失敗しました。サーバー/APIを確認してください。</Typography>)}
