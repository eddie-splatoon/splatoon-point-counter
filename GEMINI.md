...
-   **パッケージマネージャ**: pnpm を使用します。依存関係のインストールは `pnpm install`、スクリプトの実行は `pnpm run <script>` で行います。ロックファイルは `pnpm-lock.yaml` で、使用するバージョンは `package.json` の `packageManager` / `volta` フィールドで固定されています。
...
#### 設定ファイルのテスト
GitHub Issue TemplateのようなYAML形式の設定ファイルの場合、`fs`モジュールでファイルを読み込み、`js-yaml`ライブラリでパースした上で内容を検証するテストアプローチを採用しています。これにより、設定ファイルの構造と内容が期待通りであることを保証します。テストファイルは対象となる設定ファイルと同じディレクトリに配置します（例: `.github/ISSUE_TEMPLATE/2-bug-report.test.ts`）。
...
### 共通のトラブルシューティング
-   ESLintエラーが発生した場合は、まず `pnpm run lint -- --fix` を実行して自動修正を試みてください。多くの `import/order` や未使用変数などの問題はこれで解決します。

### よくあるESLintエラーとその対応

1.  **`import/order`**: インポート文の順序やグループに関するエラー。
    -   **原因**: インポート文が定義された順序規則（builtin, external, internalなど）に従っていない、またはグループ間に空行がない場合に発生します。
    -   **対応**: `pnpm run lint -- --fix` で自動修正されます。手動で修正する場合は、ESLintの設定（`eslint.config.mjs`）で定義されているグループ分けと改行規則に従ってください。
